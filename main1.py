...
class Player:
    def __init__(self, image, x, y, width, height, mu):
        self.speed = 400
    ...
    def calc_movevector(self):
        self.movevector = (self.rect.x - self.old_rect.x, self.rect.y - self.old_rect.y)
...
def manage_keys2():
    ...
    if keys[pygame.K_w]:
        ...
        wolf.slipy -= wolf.mu * MU * dt
    if keys[pygame.K_s]:
        wolf.rect.y += wolf.speed * dt
        wolf.slipy += wolf.mu * MU * dt
    if keys[pygame.K_a]:
        wolf.rect.x -= wolf.speed * dt
        wolf.slipx -= wolf.mu * MU * dt
    if keys[pygame.K_d]:
        wolf.rect.x += wolf.speed * dt
        wolf.slipx += wolf.mu * MU * dt
    ...
    if keys[pygame.K_i]:
        ...
        fox.slipy -= fox.mu * MU * dt
    if keys[pygame.K_k]:
        fox.rect.y += fox.speed * dt
        fox.slipy += fox.mu * MU * dt
    if keys[pygame.K_j]:
        fox.rect.x -= fox.speed * dt
        fox.slipx -= fox.mu * MU * dt
    if keys[pygame.K_l]:
        fox.rect.x += fox.speed * dt
        fox.slipx += fox.mu * MU * dt
    ...
    wolf.rect.x += wolf.slipx
    wolf.rect.y += wolf.slipy
    fox.rect.x += fox.slipx
    fox.rect.y += fox.slipy
...
def check_collision(rect1, rect2):
    ...
    for wall in walls:
        if fox.rect.colliderect(wall):
            fox.calc_movevector()
            fox.rect.x += -fox.movevector[0]
            fox.rect.y += -fox.movevector[1]
        if wolf.rect.colliderect(wall):
            wolf.rect = wolf.old_rect
            wolf.calc_movevector()
            wolf.rect.x += -fox.movevector[0]
            wolf.rect.y += -fox.movevector[1]

wolf = Player("wolf", random.randint(0, WIDTH), random.randint(0, HEIGHT), 30, 30, 0.2)
fox = Player("fox", random.randint(0, WIDTH), random.randint(0, HEIGHT), 30, 30, 0.5)
